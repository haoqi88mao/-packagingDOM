...
window.dom = {
  ...
  style: function style(node, name, value) {
    if (arguments.length === 3) {
      ...
      if (typeof name === "string") {
        ...
        var object = name; // dom.style(test, { border: '1px solid black', color: 'red' })
  ...
  father: function father(node) {
    return node.parentElement;
  },
  grandfather: function grandfather(node) {
  ...
  siblings: function siblings(node) {
    var x = Array.from(node.parentElement.children);
    var result = x.filter(function (n) {
      return n !== node;
    });
    return result;
  },
  ...
  index: function index(node) {
    var i = 0;
    var list = dom.children(node.parentElement);
